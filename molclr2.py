...
class MolCLR(object):
    ...
    def _get_device(self):
        if torch.cuda.is_available() and self.config['gpu'] != 'cpu':
            devices = str(self.config['gpu']).split(',')  # 支持多个 GPU，用逗号分隔
            devices = [int(device.strip()) for device in devices]  # 转换为整数列表
            print("Running on GPUs:", devices)
    ...
    def train(self):
        ...
        if 'cpu' not in self.device:
            model = torch.nn.DataParallel(model, device_ids=self.device).to(self.device[0])  # 主设备为第一个 GPU
        else:
            model = model.to('cpu')
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, (xis, xjs) in enumerate(train_loader):
                ...
                # 在 train 方法中
                xis = xis.to(self.device[0])
                xjs = xjs.to(self.device[0])
